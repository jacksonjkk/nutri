...
export const NutritionChat = ({ userProfile }: { userProfile?: UserProfile }) => {
  ...
  return (
    <Card className="flex flex-col h-[550px] max-h-[70vh] md:h-[600px] md:max-h-[80vh]">
      <div className="flex items-center justify-between mb-2 md:mb-4 border-b border-stone-100 pb-2">
        <h3 className="text-lg md:text-xl font-serif font-semibold text-stone-800">Nutri Agent</h3>
        <div className="hidden md:flex gap-2">
          <span className="text-xs text-stone-400 italic">English / Luganda / Swahili</span>
        </div>
      </div>

      <div ref={scrollRef} className="flex-1 overflow-y-auto space-y-4 mb-2 md:mb-4 pr-1 md:pr-2 scrollbar-hide">
        {messages.map((msg, i) => (
          <div key={i} className={cn(
            ...
            <div className={cn(
              ...
              {msg.image && (
                <img src={msg.image} alt="Uploaded meal" className="w-full h-40 md:h-48 object-cover rounded-xl mb-3 shadow-inner" />
              )}
      ...
      <div className="flex gap-2 pt-3 md:pt-4 border-t border-stone-100 items-center">
        <input 
          type="file" 
          accept="image/*" 
          hidden 
        ...
        <button
          ...
          className="p-2 md:p-3 text-stone-400 hover:text-[#F27D26] hover:bg-stone-50 rounded-full transition-all shrink-0"
        ...
        <div className="flex-1 relative">
          <input
            ...
            placeholder="Ask about local foods..."
            className="w-full bg-stone-50 border border-stone-200 rounded-full pl-4 pr-12 py-2.5 md:px-6 md:py-3 text-xs md:text-sm focus:outline-none focus:ring-2 focus:ring-[#5A5A40]/20 transition-all"
          />
          <button
            onClick={handleSend}
            disabled={isLoading || !input.trim()}
            className="absolute right-1 top-1 md:static bg-[#5A5A40] text-white p-2 md:p-3 rounded-full hover:bg-[#4A4A30] transition-all active:scale-90 disabled:opacity-50 shrink-0"
          >
